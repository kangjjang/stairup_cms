...
<%
	...
	int rowSize = 10;
	ArrayList<CityVO> list = dao.selectCountryList(pageno, rowSize);
	//dao = new CityDao();
	int totalcnt = dao.countryCnt();
...
<html>
	<head>
		...
		function countryView(no){
			location.href = "country_modify.jsp?countrySeqNo=" + no + "&pageno="+<%=pageno%>;
		}
		...
		<div id="wrap">
			...
			<div id="contentWrap">  
				...
				<div class="contentArea">
					<h2 class="contentTitle">국가 관리</h2>
					<div class="contentForm">
						<form name="frm" action="country_list.jsp" method="post">
								...
								<table class="hovertable" width="100%">
									<thead>
										<tr align="center">
											...
											<th style="width: 20% ;text-align: center;">국가명</th>
											<th style="width: 7% ;text-align: center;">진행순서</th>
										</tr>
									</thead>
									<tbody>		
										<%
										if(list.size() > 0){
											...
											%>
											<tr onclick="javascript:countryView(<%=vo.getCountrySeqNo() %>);" style="cursor: pointer;" <%if((i % 2) == 1 ){%> class="even"<%}%>>
												<td><%=rownum%></td>
												<td  style="text-align: center;"class="align_left"><%=vo.getCountryName() %></td>
												<td><%=vo.getOrderLy() %></td>
											</tr>
											<%
											}
						...
						<div class="alignRightButton">
					      	<button type="button" OnClick="location.href='country_add.jsp';">등록</button>
					    </div>							
					</div>
				</div>
			</div>
