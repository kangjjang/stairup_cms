...
<%
    ...
    if(vo!=null && vo.getMemResult().equals("N")){
    	obj2 = new JSONObject();
    	//dao = new MemberDao();
    	MemberVO mvo = dao.cityMayorMem(vo.getCityVo().getCitySeqNo(), memAffiliation);			//해당 도시의 시장 정보
    	//dao = new MemberDao();
    	ArrayList<MemberVO> list = dao.cityMemberList(vo.getCityVo().getCitySeqNo(), memAffiliation, memSeqNo);		//해당 도시에 있는 유저 리스트
    	if(hour > 18){
    		gubunPic = "upload/city/"+vo.getCityVo().getCityNPic();
    	}else{
    		gubunPic = "upload/city/"+vo.getCityVo().getCityPic();
    	}
    	...
    	obj2.put("memSeqNo",vo.getMemSeqNo());
    	obj2.put("memDepart", vo.getMemDepart());
    	obj2.put("departSeq", vo.getDepartSeq());
    	obj2.put("memAffiliation", vo.getMemAffiliationName());
    	obj2.put("memName", vo.getMemName());
    	if(vo.getMemPic()!=null&&!vo.getMemPic().equals("")){
    		obj2.put("memPic", "upload/memberProfile/"+vo.getMemPic());
        	obj2.put("memThumbPic","upload/Thumb/memberProfile/"+vo.getMemPic());
    	}
    	obj2.put("citySeqNo", vo.getCityVo().getCitySeqNo());
    	obj2.put("cityPic",gubunPic);
    	obj2.put("cityNum", vo.getCityVo().getOrderLy());
    	obj2.put("countryName", vo.getCityVo().getCountryName());
    	obj2.put("countryPic", "upload/country/"+vo.getCityVo().getCountryPic());
    	obj2.put("cityName", vo.getCityVo().getCityName());
    	obj2.put("memKcal",kCal);
    	obj2.put("memLife",toLife);
    	obj2.put("toDayFloor", vo.getDownCnt()+vo.getUpCnt());
    	obj2.put("toDayRank", vo.getRank());
    	...
    	if(list.size()>0){
    		...
    		obj3 = new JSONObject();
    		obj3.put("uMemSeqNo",vo.getMemSeqNo());
    		if(vo.getMemPic()!=null && !vo.getMemPic().equals("")){
    			obj3.put("uMemPic","upload/memberProfile/"+vo.getMemPic());
        		obj3.put("uMemThumbPic","upload/Thumb/memberProfile/"+vo.getMemPic());
    		}
    		list2.add(0,obj3);
    	}
    	...
    	CityVO cvo = cdao.selectMemCityInfo(memSeqNo, vo.getCityVo().getFstartDate());		//클리어 도시, 클리어 국가, 총 국가, 총 도시   
    	CityVO civo = cdao.selectNextCity(memSeqNo, vo.getCityVo().getOrderLy());			//다음도시 정보를 가져오기 위한 것
    	
    	//////System.out.println("내가 현재 머물고 있는 도시 순서 = "+ vo.getCityVo().getOrderLy());
    	...
    	ArrayList<CityVO> lvo = cdao.selectClearCityList(memSeqNo, vo.getCityVo().getFstartDate());	//거처온 국가
    	if(cvo!=null){
    		if(cvo.getClearCity() >0)percent = (int)(((double)(cvo.getClearCity()) / (double)(cvo.getTotalCity()))*100);	//(다녀온도시 - 총도시)*100 
    		
    		//임시처리 skhero.kang 2017-08-14 다음도시까지 층수가 마이너스로 나오는회원이 있음(memseq 7788)
    		nFloor = civo.getTotalStair() - (civo.getTotalWork() - (civo.getAllStair() * vo.getWorldCnt()));		//지금까지온 도시의 목표계단 총수 - 현재까지의 나의 총 층수
    		//nFloor = civo.getTotalStair();		//지금까지온 도시의 목표계단 총수 - 현재까지의 나의 총 층수
    ...
    }else if(vo.getMemResult().equals("D")){
    	resultCode = "1111";
    	resultDesc = "대기중 회원입니다.";
    }else{
    	resultCode = "9999";
